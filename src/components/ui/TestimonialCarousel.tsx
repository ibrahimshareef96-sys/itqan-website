...
export function TestimonialCarousel({ testimonials, variant = 'card' }: Props) {
  ...
  const nav = (
    <div className="flex items-center gap-2 mt-6 pt-5 border-t border-black/[0.08] dark:border-brand-cream/[0.12]">
      <button
        onClick={prev}
        className="w-8 h-8 rounded-full border border-black/[0.15] dark:border-brand-cream/25 flex items-center justify-center text-text-secondary dark:text-brand-cream/60 hover:border-brand-dark hover:text-brand-dark dark:hover:border-brand-cream dark:hover:text-brand-cream transition-colors duration-200"
  ...
  const author = (
    <div className="mt-6 flex items-center gap-3">
      ...
      <div>
        <p className="font-semibold text-text-primary dark:text-brand-cream text-sm">{current.name}</p>
        <p className="text-text-secondary dark:text-brand-cream/60 text-xs mt-0.5">{current.company}</p>
      </div>
    </div>
  );
    ...
    return (
      <div className="relative">
        <AnimatePresence mode="wait" custom={dir}>
          ...
          >
            <Quotes size={28} className={`${ACCENT} mb-4`} weight="fill" />
  ...
  return (
    <div className="bg-white dark:bg-[#2a1a28] rounded-2xl p-8 shadow-[0_8px_40px_rgba(47,28,44,0.07)] dark:shadow-[0_8px_40px_rgba(0,0,0,0.25)] border border-black/[0.08] dark:border-brand-cream/[0.12]">
      <AnimatePresence mode="wait" custom={dir}>
        <motion.div
          key={index}
          custom={dir}
          ...
          transition={{ duration: 0.38, ease: EASE_SMOOTH }}
        >
          <Quotes size={30} className={`${ACCENT} mb-5`} weight="fill" />
          <blockquote className="text-text-primary dark:text-brand-cream/90 text-base leading-relaxed">
            {current.quote}
          </blockquote>
          {author}
